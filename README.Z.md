...
1. 物料中主要是实现用户低成本开发低代码物料以组件包或cdn链接的形式。主要用basecli的docker镜像（用npm安装调用其实也可以）去使用户工程自动化构建发布为线上链接

2. 产物:产物意思应该是借助mmf原则，直接通过站点site后台的详细的接口文档json，直接映射出中后台表单页面出来，加上模板引擎填入基础依赖，运行时页面就出来了

## 26、服务端实战：服务部署

### 纯服务端部署

nestjs有2种发布类型：PM2 与 DockerCompose
